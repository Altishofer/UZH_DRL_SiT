...
def get_aug(aug_choice):
    if aug_choice == "conv_overlay":
        ...
        aug = random_overlay()
    elif aug_choice == "attmask":
        aug = random_overlay(if_mask=True)
    ...
    def parse_augs(self, aug_names):
        ...
        for each_aug_name in aug_names:
            if each_aug_name == 'all':
                selected_augs.extend(['conv','overlay','conv_overlay','rotate','shift','rotate_shift','attmask'])
    ...
    def forward(self, x, mask = None):
        ...
        for i, apply_aug in enumerate(self.augs):
            ...
            if torch.sum(selected_inds) > 0:
                if i==self.num_augs-1 and mask is not None:
                    x[selected_inds] = apply_aug(x[selected_inds],mask=mask[selected_inds])
                else:
...
class random_overlay(nn.Module):
    ...
    def __init__(self, max_batch_size=None, alpha=0.5, dataset='places365_standard',if_mask=False) -> None:
        super().__init__()
    ...
    # Loades dataset
    def load_places(self, batch_size, image_size, num_workers=2, use_val=False):
    ...
    # Iterates dataloader
    def get_places_batch(self, device):
        try:
    ...
    def forward(self, x, mask=None):
        ...
        if random_overlay.places_dataloader is None:
            ...
            random_overlay.current_imgs_buffer = self.get_places_batch(device=x.device).repeat(1, x_channel_size//3, 1, 1)
        ...
        if self.if_mask and mask is not None:
            return x*mask+ imgs*255*(1-mask)
        else:
